...
if SERVER then
	...
	function ENT:UseFunc(activator, caller, usetype, value)
		...
		if activator:IsPlayer() and caller:IsPlayer() then
			...
			local fw = self:GetAngles():Forward() * 200
			fw.z = 0
			...
			for k,v in pairs(self.Sodas) do
				if not IsValid(v) then table.RemoveByValue(self.Sodas, v) continue end
				sodas = sodas + 1
			end
			
			if sodas > 7 then activator:Notify("There are too many sodas already!", BASEWARS_NOTIFICATION_ERROR) return end
