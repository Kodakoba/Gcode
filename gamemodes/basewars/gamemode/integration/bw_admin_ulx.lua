...
function ulx.addmoney(calling_ply, target_ply, amt)
	...
	ulx.fancyLogAdmin(calling_ply, "#A gave " .. BaseWars.LANG.Currency .. "#s to #T", BaseWars.NumberFormat(amt) , affected_plys)

end

local addmoney = ulx.command(CATEGORY_NAME, "ulx addmoney", ulx.addmoney, "!addmoney")
	addmoney:addParam{type = ULib.cmds.PlayersArg}
	addmoney:addParam{type = ULib.cmds.NumArg, min = 0, max = 10^9, hint = "Money", ULib.cmds.round}
...
function ulx.takemoney(calling_ply, target_plys, amt)
	...
	ulx.fancyLogAdmin(calling_ply, "#A took " .. BaseWars.LANG.Currency .. "#s from #T", BaseWars.NumberFormat(amt), affected_plys)
...
local takemoney = ulx.command(CATEGORY_NAME, "ulx takemoney", ulx.takemoney, "!takemoney")
	...
	takemoney:addParam{type = ULib.cmds.NumArg, min = 0, max = 10^9, hint = "Money", ULib.cmds.round}
...
function ulx.setmoney(calling_ply, target_plys, amt)
	...
	ulx.fancyLogAdmin(calling_ply, "#A set #T's money to " .. BaseWars.LANG.Currency .. "#s", affected_plys, BaseWars.NumberFormat(amt))

end

local setmoney = ulx.command(CATEGORY_NAME, "ulx setmoney", ulx.setmoney, "!setmoney")
	setmoney:addParam{type = ULib.cmds.PlayersArg}
	setmoney:addParam{type = ULib.cmds.NumArg, min = 0, max = 10^9, hint = "Money", ULib.cmds.round}
...
function ulx.addlevel(calling_ply, target_plys, amt)
	...
	ulx.fancyLogAdmin(calling_ply, "#A added #s to #T's level", amt, affected_plys)
...
function ulx.addxp(calling_ply, target_plys, amt)
	...
	ulx.fancyLogAdmin(calling_ply, "#A added #s to #T's XP", amt, affected_plys)
