...
function LookEnt:Paint()
	...
	local keySize = 40
	keyUsed=keyUsed or 1
	if not offsetX||!offsetY then offsetX=0 offsetY=0 end
	local available= actionCol==Color(255,255,21)
	if available then 

		if keyMult==nil then keyMult=1 end
		keyUsed=keyUsed or 0.75
		keyMult=ValGoTo(keyMult,1,2)

		keyUsed=ValGoTo(keyUsed, 1,2) or 0
		offsetX=keyUsed*keyMult
		offsetY=keyUsed*-keyMult
		else 
		keyUsed=keyUsed or 1
		keyUsed=ValGoTo(keyUsed,0.8,2) 
		if not keyMult then keyMult=1 end
		keyMult=ValGoTo(keyMult,4.75,2)
		offsetX=keyUsed*keyMult
		offsetY=keyUsed*-keyMult
	end
	...
	surface.DrawTexturedRect(sW / 2 - keySize  - wholeW/2 - 12+offsetX, sH / 2 + 32 - offsetY, keySize*keyUsed, keySize*keyUsed)
...
LookEnt:RegisterEnt("bw_base_moneyprinter", UseBind, function(aimEnt)
	return aimEnt:GetClass()=="bw_printer_manual" and "Print" or "Collect", "money"
end,
function(aimEnt)
	return aimEnt:GetClass()=="bw_printer_manual" and color1 or aimEnt:GetMoney() > 0 and color1 or color3, color2
end)
...
LookEnt:RegisterEnt("bw_printerstorage", UseBind, function(aimEnt)
    return "Use", "Money Vault"
end,
function(aimEnt)
    return aimEnt:GetUsable() and color1 or color3, color2
end)
