...
function MODULE.FirstMoney(ply)
	...
	sql.Check(q)
	ply:SetNWFloat(tag, BaseWars.Config.StartMoney)
end
...
function MODULE.InitMoney(ply)
	...
	if not data then 
		print(ply, 'init money')
		MODULE.FirstMoney(ply)
		return puid
	end
	data = data[1]

	ply:SetNWFloat(tag, tonumber(data.money))
...
function MODULE.LoadMoney(ply)
	...
	ply:InitMoney()
	ply:SetNWFloat(tag, ply:GetMoney())
...
function MODULE.SetMoney(ply, amount)
	...
	ply:SetNWFloat(tag, tostring(amount))
