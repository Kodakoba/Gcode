...
function MODULE.Paint()
	...
	draw.DrawText(playTime(plTime), "R24", 64 + 44, sH - 112 + 6, shade, TEXT_ALIGN_LEFT)
	draw.DrawText(playTime(plTime), "R24", 64 + 42, sH - 112 + 4, white, TEXT_ALIGN_LEFT)
	...
	draw.DrawText(BaseWars.LANG.CURRENCY .. "\xe2\x80\x8a" .. money, "R24", 64 + 44, sH - 112 + 40 + 6, shade, TEXT_ALIGN_LEFT)
	draw.DrawText(BaseWars.LANG.CURRENCY .. "\xe2\x80\x8a" .. money, "R24", 64 + 42, sH - 112 + 40 + 4, moneyColor, TEXT_ALIGN_LEFT)
	
	if triggered then

		triggered = false
		local fLast = last
		local posX,posY = 64 + 45, sH - 128 + 62
		local change = 6
		local fDiff = diff
		...
		local function effectDraw(i)
			...
			draw.DrawText(
				(fDiff > 0 and "+ " .. BaseWars.LANG.CURRENCY or " " .. BaseWars.LANG.CURRENCY) .. BaseWars.NumberFormat(fDiff), "R24",
