...
function SWEP:Think()	
	...
	if self:CheckMoves(owner, dir) then self:StopDash(true) return end
 	...
 	if CLIENT then 
 		...
 		hook.Add("Move", "PlayerInDash", function(ply,m)
			...
			if not self:GetDashing() then return end
 			...
 			local dir = self:GetDashDir()
 			local moved, newdir = self:CheckMoves(owner, dir)
...
if not SERVER then return end
	...
	if not info.MovePossible then return true end
	...
	if info.InAir and dir.z < -100 and dir.z > -900 and owner:IsOnGround() then
		...
		owner:SetVelocity( Vector(dir.x * ( (1000 + dir.z) /250), dir.y * ( (1000 + dir.z) /250) , -dir.z*2  ) )
	...
	if not info.InAir and owner:Crouching() then
		...
		if not self.refilled then owner:EmitSound("dash/duckjump.ogg", 60, 100, 1, CHAN_AUTO) end
		self:SetDashDir(Vector(dir.x/1.5, dir.y/1.5, 300))
