local self = {}
GCompute.Lexing.Lexer = GCompute.MakeConstructor (self, GCompute.Lexing.ILexer)
...
function self:ctor (compilationUnit)
	...
	self.CompilerMessageSink = GCompute.NullCompilerMessageSink
	...
	self.Tokens = compilationUnit and compilationUnit:GetTokens ()
	self.Offset = 1
	self.Line = 0
	self.Character = 0
...
function self:Lex (code)
	return GCompute.Lexing.LexerTokenStream (self.Tokenizer, self.KeywordClassifier, code)
end

-- Lexer
-- These methods are all terrible and need to be destroyed.

-- Compilation
function self:GetCompilerMessageSink ()
	return self.CompilerMessageSink
end

function self:SetCompilerMessageSink (compilerMessageSink)
	compilerMessageSink = compilerMessageSink or GCompute.NullCompilerMessageSink
	if self.CompilerMessageSink == compilerMessageSink then return self end
...
function self:ProcessSome ()
	...
	self.CompilerMessageSink:Debug ("Lexer tick took " .. string.format ("%.3f", (SysTime () - self.TickStartTime) * 1000) .. " ms, now at " .. string.format ("%.2f", self:GetProgress () * 100) .. "%.")
	...
	if self:GetBytesProcessed () >= self:GetTotalBytes () then
		self.CompilerMessageSink:Debug ("Lexer took " .. ((SysTime () - self.StartTime) * 1000) .. " ms")
