...
if SERVER then
	...
	net.Receive(TagPrintOnServer,function(len,ply)
		...
		if not ply.EasyLua_ExpectingElpos or CurTime() - ply.EasyLua_ExpectingElpos > 15 then print("did not expect elpos from", ply, "; bailing") return end
...
function easylua.FindEntity(str, nowildcards)
	if not str then return NULL end

	if not nowildcards then 
		str = tostring(str)
