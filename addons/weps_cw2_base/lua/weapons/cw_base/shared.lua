...
function SWEP:Reload()
	...
	if human and self.FiremodesEnabled and ow:KeyDown(IN_USE) and self.dt.State != CW_RUNNING then
		self:CycleFiremodes()
		return
	end
	...
	if not overrideAmmoCheck then
		if ow:GetAmmoCount(self.Primary.Ammo) == 0 then
			return
		end
	end
...
local GetAimVector = function(...) reg.Player.GetAimVector(...) end -- intentional
...
function SWEP:getBaseCone()
	...
	cone_t[1], cone_t[2], cone_t[3] = aimSpreadMod or 1, hipSpreadMod or 1, maxSpreadMod or 1

	hook.Run("CW2_ConeModifiers", self, cone_t)

	aimSpreadMod, hipSpreadMod, maxSpreadMod = unpack(cone_t)

	local finalSpread = nil

	if self.dt.State == CW_AIMING then
		finalSpread = self.AimSpread * aimSpreadMod
	else
		if self.dt.BipodDeployed then
			finalSpread = self:getBipodHipSpread() * aimSpreadMod
		else
			finalSpread = self.HipSpread * hipSpreadMod
		end
	end
...
function SWEP:Think()
	...
	if human and ow:InVehicle() and not ow:GetAllowWeaponsInVehicle() then
	...
	if self.HoldToAim and human then
	...
	SWEP.BreathIntensityRegenRate = 10]]--
		...
		if self.ShotgunReloadState == 1 then
			if ow:KeyPressed(IN_ATTACK) and self:Clip1() ~= 0 then
				self.ShotgunReloadState = 2
			...
			if CT > self.ReloadDelay then
				...
				if SERVER and not SP and human then
					ow:SetAnimation(PLAYER_RELOAD)
				end

				mag, ammo = self:Clip1(), ow:GetAmmoCount(self.Primary.Ammo)

				if SERVER then
					self:SetClip1(mag + 1)
					if human then
						ow:SetAmmo(ammo - 1, self.Primary.Ammo)
					end
				end
		...
		elseif self.ShotgunReloadState == 2 then
			if ow:KeyPressed(IN_ATTACK) then
				self.ShotgunReloadState = 2
				self.ForcedReloadStop = true
			end
	...
	if (SP and SERVER) or not SP then -- if it's SP, then we run it only on the server (otherwise shit gets fucked); if it's MP we predict it
		...
		if human and not self.ReloadDelay then
...
function SWEP:PrimaryAttack()
	...
	if human and ow:KeyDown(IN_USE) then
	...
	if human and self.dt.Safe then
		self:CycleFiremodes()
		return
	end

	if human and not self:canFireWeapon(3) then
		return
	end
