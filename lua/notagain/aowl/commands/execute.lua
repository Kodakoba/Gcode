...
aowl.AddCommand("god",function(player, line)
	...
	player.GodModed = (not player:HasGodMode())
	player:SetNWBool("GodMode", player.GodModed)

	if player.GodModed then player:GodEnable() else player:GodDisable() end
	player:ChatPrint(string.format("You are now %sgodded.", (player.GodModed and "") or "un"))
end, "mods", true)